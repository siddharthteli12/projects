...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "header: {:?}, chunk: {:?}", self.header, self.chunk)
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        if value[0..8] != Self::STANDARD_HEADER {
            return Err("Invalid header type".into());
        }
        ...
        while end_index < value.len() {
            // First get length of data.
            let data_length: u32 =
                u32::from_be_bytes(value[start_index..(start_index + 4)].try_into()?);

            // End index of chunk.
            end_index = start_index + data_length as usize + constant_value;

            let chunk = Chunk::try_from(&value[start_index..end_index])?;

            result.push(chunk);

            // Update state index here to end index.
...
impl Png {
    ...
    // Only one chunk type with its data field.
    pub fn new(chunk_type: ChunkType, data: Vec<u8>) -> Self {
        Self {
            header: Self::STANDARD_HEADER,
            chunk: vec![Chunk::new(chunk_type, data)],
        }
    }
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self
            .chunk
            .iter()
            .position(|val| *val.chunk_type() == ChunkType::from_str(chunk_type).unwrap())
            .ok_or("Value not found")?;

        let result = self.chunk.remove(index);
        Ok(result)
    }
    fn header(&self) -> &[u8; 8] {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunk
            .iter()
            .find(|val| *val.chunk_type() == ChunkType::from_str(chunk_type).unwrap())
    }
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut header: Vec<u8> = self.header.to_vec();
        header.extend(self.chunk.iter().flat_map(|val| val.as_bytes()));
        header
    }
}
